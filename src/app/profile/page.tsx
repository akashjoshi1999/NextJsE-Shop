...
import { Camera, User, Lock, Save, Eye, EyeOff, X } from "lucide-react";
...
const ProfileComponent = () => {
  ...
  const handleProfileSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/profile/update`,
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        const { user: updatedUser, profileImage: updatedImage } =
          await res.json();
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/profile/password`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <div className="bg-neutral-800 rounded-2xl shadow-2xl overflow-hidden mb-8 border border-neutral-700">
          <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 px-6 py-8 relative overflow-hidden">
            ...
            <div className="flex items-center space-x-4 relative z-10">
              ...
              <div className="text-white">
                ...
                <p className="text-white/70 mt-1 drop-shadow-md">
          ...
          <div className="flex border-b border-neutral-700 bg-neutral-800">
            <button
              ...
              className={`flex-1 px-4 py-3 text-center font-medium transition-all duration-300 ${
                activeTab === "profile"
              ...
              <div className="bg-neutral-700/50 border border-neutral-600 rounded-lg p-4 backdrop-blur-sm">
                ...
                <ul className="text-sm text-neutral-400 space-y-1">
                  {(
                    ...
                    ] as { label: string; key: keyof PasswordValidation }[]
                  ).map(({ label, key }) => (
                    <li key={key} className="flex items-center space-x-2">
                      ...
                      <span
                        className={
                          passwordValidations[key] ? "text-green-400" : ""
                        }
                      >
                        {label}
                      </span>
